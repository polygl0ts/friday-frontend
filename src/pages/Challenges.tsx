...
export function Challenges() {
  ...
  const [detailsChallenge, setDetailsChallenge] = useState<ChallengeWithMeta | null>(null);
  ...
  return (
    <div className="page">
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 26, flexWrap: "wrap", gap: 16 }}>
        ...
        {isLoggedIn && (
          <div className="tier-tabs">
            {TIERS.map((t) => (
              <button key={t} className={`pill${tier === t ? " active" : ""}`} onClick={() => setTier(t)}>
                {t.toUpperCase()}
              </button>
            ))}
          </div>
      ...
      {!isLoggedIn && <div className="empty-text">Log in to view the challenges.</div>}

      {isLoggedIn && challengesQuery.isLoading && <div className="loading">Loading challenges...</div>}
      {challengesQuery.error && <div className="error-text">{(challengesQuery.error as Error).message}</div>}
